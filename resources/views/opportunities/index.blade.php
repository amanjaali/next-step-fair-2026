<x-layouts.site :title="$title" :navKey="$navKey">
    <div class="ns-wrap max-w-[1080px] pt-[clamp(28px,4vw,56px)] pb-[clamp(72px,10vw,140px)]">
        ...
        @if (! $attendee)
            {{-- Not a wall — some of what is below is already open to anyone.
                 This says what having an account adds, rather than pretending
                 nothing can be seen without one. --}}
            <div class="ns-card border-s-[6px] !border-s-magenta mb-9">
                <div class="font-[family-name:var(--ns-display)] text-[18px] font-semibold mb-2">{{ __('opportunities.guest_title') }}</div>
                <p class="ns-body !text-[14.5px] text-body-soft mb-4 max-w-[58ch]">{{ __('opportunities.guest_body') }}</p>
        ...
        @elseif ($attendee->type !== \App\Models\Registration::TYPE_STUDENT)
