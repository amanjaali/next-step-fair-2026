<x-layouts.site :title="$title" :navKey="$navKey">
    <div class="ns-wrap max-w-[620px] pt-[clamp(28px,4vw,56px)] pb-[clamp(72px,10vw,140px)]">
        ...
        <h1 class="ns-h1 !text-[clamp(30px,4.2vw,44px)] mb-3">{{ __('attendee.password.code_title') }}</h1>
        ...
        @if ($testingCode)
            <div class="border-2 border-dashed border-[#F2A93B] bg-[#FFF8EC] p-5 mb-6" role="status">
                <div class="ns-eyebrow !text-[10.5px] !text-[#8A6100] mb-2">Test mode</div>
                <span class="ns-num font-[family-name:var(--ns-display)] text-[30px] font-bold tracking-[0.18em]">{{ $testingCode }}</span>
            </div>
        @endif

        <div class="ns-card">
            <form method="POST" action="{{ route('attendee.password.verify') }}" class="flex items-end gap-4 flex-wrap">
                @csrf
                <label>
                ...
                <button type="submit" class="ns-btn ns-btn-magenta">{{ __('attendee.password.submit_request') }}</button>
            </form>

            @error('code')<span class="ns-error">{{ $message }}</span>@enderror

            <div class="mt-6 pt-5 border-t border-[rgba(5,7,8,0.12)]">
                <a href="{{ route('attendee.password.forgot') }}" class="ns-meta">{{ __('site.cta.back') }}</a>
            </div>
        </div>
    </div>
</x-layouts.site>
